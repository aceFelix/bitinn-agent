...
const fetchFavoritedArticles = async () => {
  loading.value = true
  try {
    const result = await cachedRequest('favorited-articles', {
      url: '/interaction/favorited-articles',
      method: 'GET',
    }, 3 * 60 * 1000)
    if (result.code === 200) {
  ...
  <section class="content-feed">
    <div class="feed-header">
      <div class="section-title">
        <svg viewBox="0 0 24 24" fill="none" stroke="#EAB308" stroke-width="2" width="20" height="20">
    ...
    <div class="articles-list">
      <template v-if="loading && articles.length === 0">
        <ArticleCardSkeleton v-for="i in 3" :key="'skel-' + i" rgb-color="234, 179, 8" />
      </template>
      ...
      <div v-else class="empty-state">
        ...
        <p>还没有收藏过文章</p>
        <span class="empty-hint">浏览文章时点击⭐即可收藏</span>
      </div>
    </div>
  </section>
</template>
...
.content-feed {
  ...
  &::-webkit-scrollbar-thumb { background: rgba(234, 179, 8, 0.3); border-radius: 4px; &:hover { background: #EAB308; } }
}
